...
function renderReviews(page = 1) {
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
					...
					<div class="bg-gray-400 text-neutral-content p-6 rounded-lg shadow-md">
							...
							<div class="mt-4 flex justify-between items-center">
									<button id="prevPage" class="bg-primary border p-2 rounded text-primary-content px-4 py-2 rounded hover:bg-secondary transition duration-300 ${
										page === 1 ? "opacity-50 cursor-not-allowed" : ""
									}" ${page === 1 ? "disabled" : ""}>Previous</button>
									<span>Page ${page} of ${totalPages}</span>
									<button id="nextPage" class="bg-primary border p-2 rounded text-primary-content px-4 py-2 rounded hover:bg-secondary transition duration-300 ${
	...
	// Add event listeners for rating tiles
	document.querySelectorAll(".ratingTile").forEach((tile) => {
		tile.addEventListener("click", function () {
			const rating = parseInt(this.getAttribute("data-rating"));
			document.getElementById("reviewRating").value = rating;
			updateRatingTiles(rating);
		});
	});
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
									...
									${challenges
										.map(
											...
											<li>
													...
													<div class="w-full bg-gray-400-focus rounded-full h-2.5 mt-2">
															<div class="bg-primary h-2.5 rounded-full" style="width: ${
																(challenge.progress / challenge.target) * 100
															}%"></div>
													</div>
													<div class="mt-2">
															<button class="updateChallenge text-accent hover:text-secondary mr-2 transition duration-300 border p-2 rounded" data-id="${
																challenge.id
															}">Update Progress</button>
															<button class="editChallenge text-primary hover:text-secondary mr-2 transition duration-300 border p-2 rounded" data-id="${
																challenge.id
															}">Edit</button>
															<button class="deleteChallenge text-error hover:text-secondary transition duration-300 border p-2 rounded" data-id="${
																challenge.id
															}">Delete</button>
													</div>
											</li>
					...
					<div class="bg-gray-400 text-neutral-content p-6 rounded-lg shadow-md">
							...
							<form id="addChallengeForm" class="space-y-4">
									<input type="text" id="challengeName" placeholder="Challenge Name" class="w-full p-2 border rounded text-base-content" required>
									<input type="number" id="challengeTarget" placeholder="Target (e.g., number of books)" class="w-full p-2 border rounded text-base-content" required>
									<input type="date" id="challengeEndDate" class="w-full p-2 border rounded text-base-content" required>
									<button type="submit" class="w-full bg-primary text-primary-content px-4 py-2 rounded hover:bg-secondary transition duration-300 border">Create Challenge</button>
							</form>
					</div>
			</div>
	`;
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
					<div class="bg-gray-400 text-neutral-content p-6 rounded-lg shadow-md">
							...
							<form id="searchBooksForm" class="space-y-4">
									<input type="text" id="searchQuery" placeholder="Enter book title, author, or ISBN" class="w-full p-2 border rounded text-base-content" required>
									<button type="submit" class="w-full bg-primary text-primary-content px-4 py-2 rounded hover:bg-secondary transition duration-300 border">Search</button>
							</form>
					</div>
					<div id="searchResults" class="bg-gray-400 text-neutral-content p-6 rounded-lg shadow-md">
			...
			searchBooks(query).then((results) => {
				...
				if (results.length > 0) {
					resultsContent.innerHTML = results
						.map(
							(book) => `
									<div class="mb-4 p-4 border border-neutral-focus rounded">
											<h4 class="font-semibold">${book.title}</h4>
											<p>Author: ${book.author}</p>
											<p>Published: ${book.publishedDate}</p>
											<button class="addToLibrary mt-2 bg-secondary text-secondary-content px-2 py-1 rounded hover:bg-primary transition duration-300" data-book='${JSON.stringify(
												book.volumeInfo
...
// Helper functions
function renderRatingTiles(rating) {
	return [1, 2, 3, 4, 5]
		.map(
			(i) => `
			<div class="w-6 h-6 inline-block mr-1 ${
				i <= rating ? "bg-primary" : "bg-gray-400-focus"
			}"></div>
	`
		)
		.join("");
...
function updateRatingTiles(rating) {
	document.querySelectorAll(".ratingTile").forEach((tile) => {
		const tileRating = parseInt(tile.getAttribute("data-rating"));
		tile.classList.toggle("bg-primary", tileRating <= rating);
		tile.classList.toggle("bg-gray-400-focus", tileRating > rating);
	});
}
